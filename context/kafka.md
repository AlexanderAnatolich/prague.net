...
- **Cache topics MUST have exactly one partition — not enforced in code.** The load-complete handshake is per *handler*, not per partition: `HighWatermarkOffset` is a single `long` overwritten once per assigned partition (`KafkaCacheConsumer.cs:462`), and the first partition EOF at/above it sets `IsInitialConsumeDone` and signals the countdown for the whole cache. With P > 1 that gives two defects: (a) the load is declared complete while other partitions are still behind, so their backlog is applied through the **live** path — after-handlers firing on load data, `WaitForInitialLoadAsync` returning early; (b) `_cachesLoading` is incremented per *partition* (`:464`) but decremented per *handler* (`:529`), so `CachesLoadingCount` bottoms out at P-1 and readiness (`HasIncompleteInitialLoad`) never goes healthy. `AllowAutoCreateTopics = false`, so partition count is whatever provisioned the topic — a broker default of `num.partitions=3` triggers both. Every integration-test topic is created with `partitions: 1`, so nothing covers P > 1.
