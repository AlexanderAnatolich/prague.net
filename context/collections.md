...
- **Two `Dispose` overloads:** `Dispose()` returns metadata + keys only — the values array is the hand-off payload (result maps, `SortResults`). An owner that never hands its values on (e.g. the JoinMany rounds start-round hint dictionary) must call `Dispose(withValues: true)`, otherwise a pooled values array leaks on every call.
