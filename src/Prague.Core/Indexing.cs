...
public sealed class
	...
	public CacheSymmetricKeyValueListIndex(Func<TKey, TValue, TIndexKey> keySelector, int expectedValuesPerKey = 0)
		: base(keySelector, true, expectedValuesPerKey) {
	}
...
public class CacheKeyValueListIndex<TKey, TValue, TIndexKey> : ICacheIndex<TKey, TValue>, ICountableCacheIndex
	...
	// First-generation slot hint for new buckets; 0 = PooledSet's own default.
	// Plumbed from [DataCacheIndex(ExpectedValuesPerKey = ...)] via the cache factory.
	private readonly int _bucketFirstCapacity;
	...
	protected CacheKeyValueListIndex(Func<TKey, TValue, TIndexKey> keySelector, bool reverse = false,
		...
		_bucketFirstCapacity = expectedValuesPerKey;
		if (reverse) {
			_cacheReverse = new CacheUniqueIndex<TIndexKey, TKey, TKey>(static (_, value) => value);
		}
	}

	public CacheKeyValueListIndex(Func<TKey, TValue, TIndexKey> keySelector, int expectedValuesPerKey = 0)
		: this(keySelector, false, expectedValuesPerKey) {
	}
	...
	internal CacheKeyValueListIndex(int expectedValuesPerKey = 0) {
		_bucketFirstCapacity = expectedValuesPerKey;
		KeySelector = null!;
	}
	...
	public void Update(TKey key, int keyHash, TValue orginialValue, TValue newValue, long timestampMs) {
		...
		var r = _cache.AddOrUpdate(newIndexKey,
			static (_, a) => {
				var s = new PooledSet<TKey, DefaultKeyComparer<TKey>>(default, a.Owner._bucketFirstCapacity);
				s.Add(a.Key, a.Hash);
				return s;
			},
...
public sealed class CacheCollectionSymmetricKeyValueListIndex<TKey, TValue, TIndexKey>
	...
	public CacheCollectionSymmetricKeyValueListIndex(Func<TKey, TValue, IReadOnlyList<TIndexKey>> collectionSelector,
		int expectedValuesPerKey = 0) {
		_collectionSelector = collectionSelector;
		Forward = new(expectedValuesPerKey);
		Reverse = new(expectedValuesPerKey);
	}
