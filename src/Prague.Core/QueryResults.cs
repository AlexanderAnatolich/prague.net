...
public readonly struct QueryResults<T> : IList<T>, IReadOnlyList<T>, IDisposable, IQueryResults {
	...
	/// <summary>
	///   Stable sort: rows the comparer calls equal keep their encounter order. The bounded top-K
	///   plan behind the paged terminals breaks ties by encounter ordinal, so the unbounded sort has
	///   to as well — otherwise Execute() and Execute(skip, take) disagree on the order of equal rows
	///   over identical data, and consecutive pages stop partitioning the result.
	///   Sorts a rented index array with the ordinal as the final tiebreak, then applies the
	///   permutation in place by cycle-following: one pooled int per row, no element copies beyond
	///   the cycles themselves.
	/// </summary>
	public void SortStable<TComparer>(TComparer comparer) where TComparer : IComparer<T> {
		var span = AsSpan();
		if (span.Length < 2)
			return;

		var rented = PragueArrayPool<int>.Pool.Rent(span.Length);
		try {
			var order = rented.AsSpan(0, span.Length);
			for (var i = 0; i < order.Length; i++)
				order[i] = i;

			order.Sort(new OrdinalTiebreakComparer<TComparer>(_array!, _offset, comparer));

			// Apply result[i] = span[order[i]] in place: lift the row at the start of each cycle,
			// shift the cycle's rows down one slot, drop the lifted row in the hole that closes it.
			// order[j] is reset to j as the cycle is consumed, so every slot is visited once.
			for (var i = 0; i < order.Length; i++) {
				if (order[i] == i)
					continue;
	...
	// which is the row's encounter ordinal — a total order, so the underlying unstable sort cannot
	// reorder equal rows.
	private readonly struct OrdinalTiebreakComparer<TComparer> : IComparer<int>
		...
		public OrdinalTiebreakComparer(T[] items, int offset, TComparer comparer) {
			_items = items;
			_offset = offset;
			_comparer = comparer;
		}
