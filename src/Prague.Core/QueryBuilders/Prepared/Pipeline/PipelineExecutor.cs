...
internal readonly struct PipelineCore<TKey, TValue, TArgs>
	...
	private bool ChooseSeed(IPipelineStep<TKey, TValue, TArgs>[] steps, scoped ref PipelineFrame<TKey> frame, bool free) {
		...
		_plan.Record(mode, seed, seedSignal, orUnion);

		// A walked step is judged by its index. A step a free seed moved off the first one keeps its
		// value-side probe when it has one, so the rows it admits are judged as the fixed walk would judge
		// them — by the value returned (§14.1) — at one field compare per survivor. A key-side walk is its
		// own probe. An Or that seeds keeps its probe when a branch has more than one leaf (its union is a
		// superset) or when it was moved off the first step.
		var keyProbes = 0;
		var valueProbes = 0;
