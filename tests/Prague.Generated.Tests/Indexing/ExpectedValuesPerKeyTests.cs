...
/// <summary>
///   End-to-end for the [DataCacheIndex(ExpectedValuesPerKey = ...)] sizing hint:
///   attribute → source generator → Cache.CacheKeyValueListIndex(..., expectedValuesPerKey)
///   → per-key bucket PooledSet first-generation capacity. Unhinted indexes keep the
///   historic 127-slot default; the hint shrinks per-entity indexes whose buckets are
///   known to hold only a handful of values.
/// </summary>
[TestFixture]
public class ExpectedValuesPerKeyTests {
	...
	public void HintedIndex_FirstBucketGeneration_HonorsTheHint() {
		...
		Assert.That(plain, Is.EqualTo(127),
			"an unhinted index must keep the historic default bucket geometry");
		Assert.That(hinted, Is.GreaterThanOrEqualTo(64),
			"the hinted bucket must rent at least ExpectedValuesPerKey slots");
