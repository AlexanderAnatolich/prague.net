...
public class PooledSetLazyInitTests {
	...
	[Test]
	public void FirstAdd_RentsTheHistoricDefaultGeneration_StableUntilFull() {
		var set = new PooledSet<long, DefaultKeyComparer<long>>();
		...
		Assert.That(set.CapacitySlots, Is.EqualTo(127),
			"unhinted sets must keep the exact pre-lazy default table geometry");

		// The default generation serves up to 127 entries without another swap.
		for (long i = 2; i <= 127; i++) {
		...
		Assert.That(set.Add(128), Is.True); // exceeds the first generation -> grow
		...
		Assert.That(set.Count, Is.EqualTo(128));
	...
	[Test]
	public void FirstCapacityHint_ShrinksTheFirstGeneration_ForSmallBuckets() {
		// The per-entity bucket case (a handful of values per index key): the hint
		// replaces the 127-slot default with a prime-rounded small first table.
		var set = new PooledSet<long, DefaultKeyComparer<long>>(default, firstCapacity: 8);
		...
		Assert.That(set.CapacitySlots, Is.LessThan(127),
